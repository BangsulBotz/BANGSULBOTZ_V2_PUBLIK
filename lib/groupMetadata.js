...
import { isSetInfoEnabled, isInfoLabelEnabled, isWelcomeEnabled } from './group_handler.js';
...
export function registerGroupMetadataListeners(sock) {
    ...
    // --- 2. LISTENER PERUBAHAN GRUP (groups.update) ---
    ...
    // --- 3. LISTENER FOTO PROFIL GRUP (contacts.update) ---
    sock.ev.on('contacts.update', async (updates) => {
        for (const update of updates) {
            ...
            // Coba ambil author dari metadata grup
            let authorJid = '';
            let authorTag = 'Admin';
            try {
    ...
    // --- 4. LISTENER UPDATE PESERTA + WELCOME ---
    sock.ev.on('group-participants.update', async (update) => {
        ...
        if (update.action === 'add' || update.action === 'remove') {
            ...
            try {
                ...
                const welcomeText = `👥 *UPDATE PESERTA GRUP*\n\n${actionText}`;
        ...
        if ((update.action === 'promote' || update.action === 'demote') && isSetInfoOn) {
            const updateText = `👥 *UPDATE PESERTA GRUP*\n\n${actionText}`;
            let ppUrl = null;
...
export async function handleGroupSystemMessages(sock, msg) {
    ...
    sock.ev.on('chats.update', async (updates) => {
        for (const update of updates) {
            ...
            if (update.ephemeralExpiration === undefined) continue;
