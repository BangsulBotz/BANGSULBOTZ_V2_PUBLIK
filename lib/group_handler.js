...
const DEFAULT_CONFIG = {
    ...
    infolabel: 1,
    autogemini: 0,
    antihidetag: { status: false, delete: false, kick: false },
...
// Fungsi pengecekan status fitur
...
export const isInfoLabelEnabled = (jid) => checkStatus(jid, 'infolabel');
export const isFeatureEnabled = (jid, feature) => checkStatus(jid, feature);


// Eksekutor sanksi (Hapus/Kick)
...
/**
 * Advanced Bot Detection Logic (Clean Version)
 */
...
export async function groupUpdateHandler(sock, update) {
    ...
    // --- [ AUTO-SYNC DATABASE (SQLite Version) ] ---
    // Update subject grup di DB jika terdeteksi ada perubahan nama
    ...
    if (!protocolMsg && msg.isGroup && msg.isBotAdmin && !msg.isOwner) {
        ...
        if (configSticker?.status && !msg.isAdmin) {
            // Deteksi sticker dari berbagai kemungkinan struktur Baileys
            const isSticker = 
        ...
        if (configTagAll?.status) {
            ...
            // Deteksi nonJidMentions: 1 (Hidetag detected)
...
export const isAutoAIEnabled = (jid) => {
    ...
    try {
        ...
        // Karena di DB kamu pakai 0 / 1 (number)
